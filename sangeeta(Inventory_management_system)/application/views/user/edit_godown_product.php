...
<form action="<?php echo base_url(); ?>user/product_edit_save" method="post">
    <table  cellspacing="1" width="99%" cellspacing="0" border="0" align="left" id="tblDisplay">
        <tbody>
            ...
            <input type="hidden" name="sub_category_name" value="<?php echo $sub_category_name?>">
                <?php
            for($i=0;$i<$count;$i++){
                $q1=mysql_query("select * from tbl_godown where godown_id='$godown_id[$i]'");
                $fetch1=mysql_fetch_array($q1);
                        ?>
            <tr bgcolor="#003366">
                <input type="hidden" name="godown_id[]" value="<?php echo $godown_id[$i]; ?>">
